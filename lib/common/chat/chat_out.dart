...
class ChatOutCommon with Tag {
  ...
  Future<List<dynamic>> _sendData(List<String> destList, String data, {bool ping = false, bool lastTime = false, int maxHoldingSeconds = 8640000}) async {
    ...
    try {
      ...
      if (errStr.contains(NknError.invalidDestination)) {
        ...
        return [null, false, 0];
      } else if (NknError.isClientError(e)) {
  ...
  Future<OnMessage?> sendMsg(List<String> destList, String data, {int maxHoldingSeconds = 8640000}) async {
    ...
    if (data.length >= Settings.sizeMsgMax) {
      ...
      // Sentry.captureMessage("$TAG - sendMsg - size over - size:${Format.flowSize(data.length.toDouble(), unitArr: ['B', 'KB', 'MB', 'GB'])} - destList:$destList");
      // return null;
    }
    // send
  ...
  Future<List<dynamic>> _sendPieces(List<String> clientAddressList, MessageSchema message, {double totalPercent = -1}) async {
    ...
    if (dataList.isEmpty) {
      logger.e("$TAG - _sendPieces:ERROR - total:$total - parity:$parity - bytesLength:${Format.flowSize(bytesLength.toDouble(), unitArr: ['B', 'KB', 'MB', 'GB'])} - parentType:${message.contentType}");
      return [null, false];
    }
    logger.i("$TAG - _sendPieces:START - total:$total - parity:$parity - bytesLength:${Format.flowSize(bytesLength.toDouble(), unitArr: ['B', 'KB', 'MB', 'GB'])} - parentType:${message.contentType}");
    ...
    if (finds.length >= total) {
      logger.i("$TAG - _sendPieces:SUCCESS - count:${resultList.length} - total:$total - parity:$parity - bytesLength:${Format.flowSize(bytesLength.toDouble(), unitArr: ['B', 'KB', 'MB', 'GB'])} - parentType:${message.contentType} - message:${message.toStringSimple()}");
      if (finds.isNotEmpty) return [finds.firstWhere((element) => element.pid != null).pid, true];
    }
    logger.w("$TAG - _sendPieces:FAIL - count:${resultList.length} - total:$total - parity:$parity - bytesLength:${Format.flowSize(bytesLength.toDouble(), unitArr: ['B', 'KB', 'MB', 'GB'])} - parentType:${message.contentType} - message:${message.toStringSimple()}");
    return [null, false];
  }
  ...
  Future<Uint8List?> _sendWithContact(ContactSchema? contact, MessageSchema? message, {bool notification = false, int maxHoldingSeconds = 8640000}) async {
    ...
    if (message.canTryPiece) {
      try {
        List result = await _sendPieces([message.targetId], message);
        pid = result[0];
        tryNoPiece = result[1];
      } catch (e, st) {
        handleError(e, st);
  ...
  Future<Uint8List?> _sendWithTopic(TopicSchema? topic, MessageSchema? message, {bool notification = false}) async {
    ...
    if (destList.isNotEmpty) {
      ...
      if (message.canTryPiece) {
        try {
          List result = await _sendPieces(destList, message);
          pid = result[0];
          tryNoPiece = result[1];
        } catch (e, st) {
          handleError(e, st);
  ...
  Future<MessageSchema?> _sendVisible(
    ...
    if (sendSuccess) {
      ...
      logger.w("$TAG - _sendVisible - pid is null - type:${message.contentType} - message:${message.toStringSimple()}");
    }
  ...
  Future<MessageSchema?> resend(String? msgId, {bool mute = false, int muteGap = 0}) async {
    ...
    if (mute) {
      ...
      // send_mute
      MessageSchema? result = await _sendVisible(message, insert: false, sessionSync: false, statusSync: false, notification: notification);
      if (result != null) {
    ...
    // send
    return await _sendVisible(message, insert: false);
  }

  // NO DB NO display NO topic (1 to 1)
  Future<int> sendPing(List<String> clientAddressList, bool isPing, {int gap = 0}) async {
    if (!(await clientCommon.checkClientOk("sendPing"))) return 0; // need client.address
  ...
  Future<bool> sendReceipt(MessageSchema received) async {
    if (received.isTargetTopic || received.isTargetGroup) {
      ...
      return true; // topic/group no receipt, just send message to myself
    }
  ...
  Future<bool> sendContactOptionsBurn(String? targetAddress, int deleteSeconds, int updateAt) async {
    ...
    var result = await _sendVisible(message);
    return result != null;
  }
